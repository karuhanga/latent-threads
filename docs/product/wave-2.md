...
Reuse entities only when meanings match. At least two researched concepts/skills should connect genuinely distinct verticals, with visible paths proving reuse. Do not manufacture universal role collaboration or an organization-to-organization supply chain. Keep current uncluttered UI, global discovery and independent entities. Catalog search ranks exact labels first, exact aliases next and partial matches afterward, so a vertical is findable by its common name. Learning prerequisites that affect starting a resource remain visible with source details off.
