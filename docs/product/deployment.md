...
**Preflight limits:** local package installation, build, browser preview and Git pushes passed. The manual custom Actions preflight failed before a hosted runner started; account-owner action is required for that route. This does not establish that branch publishing is blocked. GitHub documents `.nojekyll` publishing without a Jekyll build, while its current publishing guide says Pages still uses a managed deployment workflow. Test the actual branch route before requiring account repair for deployment; do not promise that it bypasses account restrictions. No branch deployment has been attempted yet. [Pages publishing sources](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site), [GitHub's legacy-worker announcement](https://github.blog/changelog/2024-07-08-pages-legacy-worker-sunset/).
