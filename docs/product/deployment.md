...
- Local Git: reviewed code/docs pushed to main; authenticated system Git publishes gh-pages.
...
- Subagents: bounded implementation and independent review completed; no builder remains active.
- Browser inspection: real local and hosted keyboard/mobile/deep-link walkthroughs completed.
...
Use the exact versions committed in `.node-version` and `package.json` (currently Node 24.19.0 and pnpm 11.19.0), with `node`, `pnpm` and the authenticated system `git` on `PATH`. On this machine use the README PATH order, placing `/usr/bin:/bin` ahead of the bundled fallback tools: fallback/git uses a different authentication route and failed public-branch pushes. The first checked deployment was pushed with system Git after the scripted fallback Git failed; final release verified the corrected PATH end to end, without a manual push workaround. Git must have an author name/email and working push access to the configured `origin`. The publisher accepts only the existing `karuhanga/latent-threads` repository. Commit the reviewed source first: tracked modifications and untracked files both stop publication; ignored local files such as `dist/` do not.
...
This rebuilds the same clean committed source, fetches `gh-pages` into another owned temporary checkout, and creates a deployment commit whose parent is the previous deployment. Its commit message and `release.json` identify the source revision. The first deployment creates the branch without source history. The deployment commit is imported through an owned temporary ref into the original checkout, whose existing Git authentication performs the ordinary, non-force push. The temporary ref is removed; HEAD, worktree and FETCH_HEAD are preserved. Only remote `gh-pages` is updated; a competing remote update causes rejection and requires a fresh run. Identical output at the existing deployment creates no duplicate commit. The command does not enable Pages or change repository settings: configure `gh-pages` and `/ (root)` once through GitHub, then verify the resulting live page and a refreshed deep link before marking LT-011 complete.
