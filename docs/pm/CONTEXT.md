...
- **App foundation is in review; no verified deployment yet.** LT-003 agent reports install/typecheck/three routing tests/build passed. PM reviewed code and verified real subpath preview: an exploration link survives refresh and malformed #/unknown recovers to home. The foundation is not the visual shell or sourced release; fresh-directory installation verification is completing.
...
**Next action:** review LT-003 scaffold, then implement LT-004 visual shell and show Checkpoint A while LT-006 curation progresses. LT-005 schema follows scaffold acceptance. Live dependencies were checked before dispatch; at most two builders are active.
