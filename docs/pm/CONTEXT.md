...
- PM reports that `git ls-remote` succeeded and the browser is signed in to GitHub. The `gh` CLI itself is unauthenticated; alternative Git/browser routes are available, so CLI authentication is **not** a current user blocker. Recheck the relevant route before a remote write; successful reads alone do not establish every write permission.
- PM reviewed the ontology and clarified Wave 2 geography UI. **LT-002 Done; LT-003 scaffold Ready; LT-006 song curation Ready** at this snapshot. Fetch live Notion status before acting.
